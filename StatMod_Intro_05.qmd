...
Der Standardfehler (engl.: "**standard error**" oder **se**) ist ein Maß dafür, wie stark eine Zusammenfassung der Stichprobe &ndash; ein Schätzwert &ndash; (z.B. der Mittelwert) vom wahren, unbekannten Wert der Population abweicht. Je größer die Stichprobe ($n$), desto kleiner der Standardfehler:

$$ se = \frac{sd}{\sqrt{n}} $$

<br>

[Nicht]{.red} zu verwechseln mit: **Standardabweichung**

- Die **Standardabweichung** ist ein Maß für die Streuung der Beobachtungen, das heißt wie gleich oder verschiedenarting die Beobachtungen in der Stichprobe sind.


## Konfidenzintervall

Ein **Konfidenzintervall** gibt einen Bereich an, der den wahren, unbekannten Wert der Population mit einer gegebenen Sicherheit ($1 - Irrtumswahrscheinlichkeit \ (\alpha)$) überdeckt, d.h., den Anteil der so konstruierten Konfidenzintervalle, die den Wert enthalten. Für den Stichprobenmittelwert ergeben sich folgende Grenzen:
...
- Je größer der Stichprobenumfang, desto schmaler das Konfidenzintervall (unter sonst gleichen Umständen): Der Standardfehler $se$ fällt mit steigenden $n$.
...
Der **p-Wert** gibt den Anteil der Stichproben an, die ein midenstens genau so extremes Ergebnis wie die beobachtete Sichprobe haben, unter der Annhame, dass die $H_0$ gilt.
...
<br>
**Testentscheidung**: <br>
(**Keine** Entscheidung sollte rein auf Basis den p-Wertes getroffen werden, *vor* der Testentscheidung **immer** eine explorative Datenanalyse durchführen.)
...
$${y_i} = {\beta_0} + {\beta_1} \cdot {x_i1} + {\beta_2} \cdot {x_i2} + ... + {\beta_p} \cdot {x_ip} + {\epsilon_i} $$
...
$${y_i} = {\beta_0} + {\beta_1} \cdot {x_i1} + {\beta_1} \cdot {x_i1} + {\beta_3} \cdot ({x_i1} \cdot {x_i2}) + {\epsilon_i} $$
